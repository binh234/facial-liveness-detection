...
class CustomDataGen(tf.keras.utils.Sequence):
    ...
    def __get_data(self, batches):
        ...
        X0_batch = np.asarray([self.__get_input(x, self.input_size)
                              for x in img1_path_batch])
        X1_batch = np.asarray([self.__get_input(x, self.input_size)
                              for x in img2_path_batch])
